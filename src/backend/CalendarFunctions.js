...
function preview(meeting, start) {
    ...
    const busyTimes = getFreebusyTimes(meeting, { start: startWindow, end: endWindow });

    let possibleEvents = [];
    let proposedEvent = {"start": startWindow, "end": new Date(startWindow.getTime() + (meeting.duration * 60000))};
...
function scheduleEvent(meeting, startTime, guestEmail) {
    ...
    let event = calendar.createEvent(meeting.title, startTime, new Date((startTime.getTime() + meeting.duration * 60000)));
...
function isNotDuringBusy(busyTimes, proposedEvent, officeHours) {
    for(var i = 0; i < busyTimes.length; i++) {
        ...
        if(proposedEvent.start.getTime() >= busyTimes[i].start.getTime() &&
            proposedEvent.start.getTime() < busyTimes[i].end.getTime() ||
        ...
        if((proposedEvent.end.getTime() > busyTimes[i].start.getTime() &&
                proposedEvent.end.getTime() <= busyTimes[i].end.getTime()) ||
...
function getCalendar(calendarName) {
    ...
    calendar = calendar != null ? calendar : CalendarApp.getCalendarById(calendarName);
...
function relativeTimeWindowForHost(timeZone, startTime) {
    ...
    var offset = 24 - new Date(Date.parse(formatter.format(start))).getHours();
