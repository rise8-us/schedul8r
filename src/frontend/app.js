...
function updateSettings(newSettings) {
  ...
  $("#meetingDuration")
    ...
    .append(`${newSettings.duration} min`);
  $("#interviewer")
    .empty()
    .removeClass("skeleton")
    .append(newSettings.displayName);
}
...
`;
  ...
  for (const id of ids) {
    document.getElementById(id).addEventListener("click", (event) => {
      const [newYear, newMonth] = calculateNewYearAndMonth(
        currentYear,
        currentMonth,
        id
      );
...
function generateDays(maxWeeks, yearInt, monthInt, firstDayOfWeek) {
  ...
  for (let i = 0; i < maxWeeks; i++) {
    ...
    for (let j = 1; j <= 7; j++) {
      ...
      const id = [
        day.getFullYear(),
        guaranteeTwoDigits(day.getMonth()),
        guaranteeTwoDigits(day.getDate()),
      ].join("-");
      let disabled = "";

      buttonIds.push(id);
      if (dateCheck < 1 || [0, 6].some((weekend) => day.getDay() === weekend))
        disabled = "disabled";
...
function generateHourButtons(data) {
  ...
  for (let timeBlock of data) {
    const displayTime = new Date(timeBlock)
      .toString()
      .split(" ")[4]
      .substring(0, 5);
  ...
  `);
    ...
    button.addEventListener("click", (event) => {
      ...
      confirmBtn.append(
        '<button id="timeConfirmButton" class="time_confirm_btn">Confirm</button>'
      );
...
function generateRequestorForm(timeBlock) {
  ...
  $("#date-time-value")
    .empty()
    .append(buildDurationString(timeBlock, meetingSettings.duration));

  document
    .getElementById("scheduleEventButton")
    .addEventListener("click", (event) => {
      createRipple(event);
...
function addTimezone() {
  return `
    ${getGlobeIcon()}
    <h4 class="info">${Intl.DateTimeFormat()
      .resolvedOptions()
      .timeZone.replace("/", " / ")
      .replace("_", " ")}</h4>
`;
