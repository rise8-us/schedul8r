...
google.script.run.withSuccessHandler(updateSettings).withFailureHandler((error) => console.error(error))
.getMeetingInfo(parameter.meetingType);
}
...
children +=(`<h6 class="weekday">${weekdays[i]}</h6>`);
}
...
google.script.run.withSuccessHandler(generateHourButtons).withFailureHandler((error) => console.error(error))
...
displayTime = new Date(timeBlock).toString().split(" ")[4].substring(0,5);
...
document
.getElementById("timeConfirmButton")
.addEventListener("click", event => {
createRipple(event);
...
document.getElementById('scheduleEventButton')
.addEventListener('click', (event) => {
createRipple(event);
...
if (shouldSchedule) {
google.script.run
.withSuccessHandler(() => showSuccessMark(requestorName.value, meetingSettings.displayName, requestorEmail.value))
...
return `
    ${getCalendarIcon()}
    <h4 id="date-time-value" class="info">To Be Determined</h4>
`;
}
...
dateToCompareAsNumber = +(dateToCompare.getFullYear() + monthString + dayString);
...
function buildDurationString (start) {
const startTime = (new Date(start).toTimeString().substring(0, 5));
const end = new Date(start + (meetingSettings.duration * 60000));
const endTime = end.toTimeString().substring(0, 5);

return `${startTime} - ${endTime}, ${weekday[end.getDay()]}, ${months[end.getMonth()]} ${end.getDate()}, ${end.getFullYear()}`;
}
